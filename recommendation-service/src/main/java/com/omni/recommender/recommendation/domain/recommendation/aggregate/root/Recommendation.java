...
public class Recommendation {
    ...
    private Recommendation(TenantId tenantId, UserId userId, List<RecommendedItem> items, Instant generatedAt, boolean isFallback) {
     ...
     * 從資料庫/Redis 重建模型
     */
    public static Recommendation restore(TenantId tenantId, UserId userId, List<RecommendedItem> items, Instant generatedAt) {
        return new Recommendation(tenantId, userId, items, generatedAt, false);
    }

    /**
     * 建立預設/熱門商品的降級模型 (Cache Miss Fallback)
     */
    public static Recommendation fallback(TenantId tenantId, UserId userId, List<RecommendedItem> defaultItems) {
        return new Recommendation(tenantId, userId, defaultItems, Instant.now(), true);
    }
