...
## 📌 [Epic] 擴充與拆分微服務 (Microservices Extraction)
...
### 📖 背景與問題痛點
目前系統的核心邏輯高度集中於 `product-service` 與 `order-service` 中，包含了庫存預扣與付款邏輯，這會導致高流量下各模組互相影響（例如：商品瀏覽的高流量影響了結帳的穩定性）。
...
1. **真正的即時性與徹底解耦**: API 伺服器只需對 Kafka "Fire and Forget" (射後不理)，完全將 HDFS 的可用性與硬碟 I/O 從使用者的請求關鍵路徑中移除，大幅降低延遲。
   > **架構決策 (ADR)**: 針對 `behavior-service`，由於其處理的是高吞吐、容忍微小遺失的行為日誌，我們**不採用 Outbox Pattern**。直接發送 Kafka 以追求極致效能，避免資料庫成為瓶頸。
2. **削峰填谷 (Load Leveling)**: Kafka 能在流量突增（如雙 11 促銷）時作為巨大的緩衝區，保護後端的 HDFS 或是未來的 MinIO 不被龐大流量壓垮。
3. **一源多用 (Multi-Consumer)**: 單一的 Kafka Topic 可以完美實現 Lambda/Kappa 雙軌架構：
   - *消費者 1 (批次)*: 透過 Kafka Connect (MinIO/S3 Sink) 將資料倒進 Data Lake，留給 Spark ALS 做離線模型訓練。
   - *消費者 2 (即時)*: 透過 Flink 或 Spark Streaming 即時消費事件，用於更新即時戰情儀表板。
...
flowchart TD
    ...
    Spark -- "6. 計算 ALS 模型並寫入推薦清單" --> Redis
    
    Client -- "7. GET /recommendations/{userId}" --> RES
    RES -- "8. 亞毫秒級查詢推薦結果" --> Redis
```

## 📌 [已完成] 領域模型 (Domain Model) 擴充與完善
- [x] **多租戶支援 (Multi-Tenancy)**: 於 `UserBehavior` 核心 Aggregate Root 中加入 `TenantId`，並貫穿至 REST DTO 與 Command，實現服務等級的租戶隔離。
- [x] **高價值行為意圖擴充**: 整理 `BehaviorType` 枚舉，替換 `LIKE` 為 `FAVORITE` 以符合電商語意，並擴充 `UNFAVORITE`, `REMOVE_FROM_CART`, `CHECKOUT`, `RATE`, `SHARE`，完善購物漏斗的特徵捕捉。
- [x] **跨服務打點整合**: 在 `product-service` 中實作非同步的 `BehaviorTrackingService` (基於 Spring 6 HTTP Interfaces)，成功將使用者的「瀏覽」與「加入最愛」事件拋送至 `behavior-service`。

*(未來的其他待辦事項與優化計畫將會陸續補充於此)*

---

## 📌 [Epic] 實作客製化費用計算邏輯 (Tax & Shipping)

**狀態**: 📋 待辦 (Backlog)  
**優先級**: 🌟 中 (待基本結帳流程穩定後進行)  
**領域**: 訂單與計價領域 (Order & Pricing Domain)

### 📖 背景與問題痛點
目前的結帳流程中，尚未包含運費 (Shipping) 與稅金 (Tax) 的動態計算。這兩項費用的計算往往高度依賴使用者的所在地、配送方式，或是購物車的總金額（例如滿額免運）。若將這些計算邏輯放在 `ProductService` 會打破領域邊界，因為商品服務不該知道「寄送地址」或「訂單金額」等上下文。

### 🎯 解決方案
應將客製化費用的計算邏輯實作於 `OrderService` 的結帳流程中，確保職責分離。未來若計價規則變得極度複雜，可進一步拆分出獨立的計價微服務。

### 🛠️ 具體實作步驟
1. **初期實作 (在 OrderService 內)**:
   - 在 `OrderService` 內建立 `PricingDomainService` 或基於策略模式 (Strategy Pattern) 的 `FeeCalculator`。
   - 建立不同地區的稅率策略 (`TaxCalculationStrategy`) 與運費計算規則 (`ShippingFeePolicy`)。
